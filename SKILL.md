# Skill: Equipo OctaneBot (Multi-Agente)
...
1. **Director (OctaneBot)**: El líder. Divide el problema, asigna roles y aprueba planes.
...
Antes de realizar cambios significativos, cada agente debe enviar un **Plan de Acción** al buzón de OctaneBot.

- El agente se mantiene en modo `READ_ONLY` o `PLANNING` hasta que OctaneBot responda con un mensaje de `APPROVED`.
...
- **Broadcast**: OctaneBot puede escribir en `broadcast.msg` para dar nuevas directrices a todo el equipo simultáneamente.
...
- Al completar una tarea, el agente debe liberar sus "locks" y notificar a OctaneBot.
...
def init_team():
    ...
    print("✓ Infraestructura 'Equipo OctaneBot' lista.")
